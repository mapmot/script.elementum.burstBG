...
import dns.rdata
from dns._compat import int, xrange, round_py2_compat


_pows = tuple(int(10**i) for i in range(0, 11))
...
class LOC(dns.rdata.Rdata):
    ...
    def to_wire(self, file, compress=None, origin=None):
        ...
        latitude = int(0x80000000) + milliseconds
        milliseconds = (self.longitude[0] * 3600000 +
                        self.longitude[1] * 60000 +
                        self.longitude[2] * 1000 +
                        self.longitude[3]) * self.longitude[4]
        longitude = int(0x80000000) + milliseconds
        altitude = int(self.altitude) + int(10000000)
    ...
    def from_wire(cls, rdclass, rdtype, wire, current, rdlen, origin=None):
        ...
        if latitude > int(0x80000000):
            latitude = float(latitude - int(0x80000000)) / 3600000
        else:
            latitude = -1 * float(int(0x80000000) - latitude) / 3600000
        if latitude < -90.0 or latitude > 90.0:
            raise dns.exception.FormError("bad latitude")
        if longitude > int(0x80000000):
            longitude = float(longitude - int(0x80000000)) / 3600000
        else:
            longitude = -1 * float(int(0x80000000) - longitude) / 3600000
